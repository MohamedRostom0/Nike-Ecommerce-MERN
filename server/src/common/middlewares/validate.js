...
export const validate = (schema, options) => {
  ...
  return (req, res, next) => {
    ...
    if (!_.isEmpty(paramsSchema)) {
      const { value: validatedParams, error } = compiledParamsSchema.validate(
        req.params,
        validationOptions
      );
    ...
    if (!_.isEmpty(querySchema)) {
      const { value: validatedQuery, error } = compiledQuerySchema.validate(
        req.query,
        validationOptions
      );
    ...
    if (!_.isEmpty(bodySchema)) {
      const { value: validatedBody, error } = compiledBodySchema.validate(
        req.body,
        validationOptions
      );
    ...
    if (!_.isEmpty(fileSchema)) {
      const { value: validatedFile, error } = compiledFileSchema
        .required()
